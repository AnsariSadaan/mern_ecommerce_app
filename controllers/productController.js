    ...
    try {
        ...
        switch (true) {
            ...
            case !photo && photo.size > 100000:
                return res.status(500).send({error: "Phonto is required and should be less than 1mb"});
        }
        // const errors = [
        //     { condition: !name, message: "Name is required" },
        //     { condition: !description, message: "Description is required" },
        //     { condition: !price, message: "Price is required" },
        //     { condition: !category, message: "Category is required" },
        //     { condition: !quantity, message: "Quantity is required" },
        //     { condition: !photo || (photo.size > 100000), message: "Photo is required and should be less than 1mb" }
        // ];
...
// update product
export const updateProductCotroller = async (req, res)=>{
    try {
        const { name, slug, description, price, category, quantity, shipping } = req.fields
        const { photo } = req.files
        // validation
...
export const getProductController = async (req, res)=>{
    try {
        ...
        res.status(500).send({
            success: false,
            error: error.message,
            message: "Error ehile getting all products"
        })
    }
}
